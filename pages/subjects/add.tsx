...
interface FormikInitialValues {
  subject: { sem: string; courseId: string; batchId: string };
...
export default function CreateResult({ data }: { data: Subject[] }) {
  ...
  const createSubjects = async (
    ...
    const subjects = values.subjects.map((subject) => {
      return {
        ...subject,
        batchId: Number.parseInt(values.subject.batchId),
        courseId: Number.parseInt(values.subject.courseId),
        sem: values.subject.sem,
      };
    });
    NProgress.start();
    await axios
